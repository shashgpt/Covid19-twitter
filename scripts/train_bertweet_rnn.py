...
class BERTweet_rnn(tf.keras.Model):
    ...
    def call(self, input_ids, attention_masks=None, **kwargs):
        ...
        #rnn
        if self.config["model_name"] == "bertweet_lstm":
            rnn_output = self.lstm(word_embeddings)
        elif self.config["model_name"] == "bertweet_bilstm":
            rnn_output = self.bilstm(word_embeddings)
        elif self.config["model_name"] == "bertweet_gru":
            rnn_output = self.gru(word_embeddings)
        elif self.config["model_name"] == "bertweet_bigru":
...
class train_bertweet_rnn(object):
    ...
    def train_model(self, train_dataset, val_datasets, test_datasets):
        ...
        model = BERTweet_rnn(self.config).build_model(input_shape = train_dataset[0].shape[1])
        self.model = model
