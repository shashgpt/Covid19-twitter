...
class distilbert_transformer(tf.keras.Model):
    def __init__(self, config, num_heads, maxlen, epsilon, **kwargs):
        ...
        self.positional_embedding_layer = PositionalEmbedding(maxlen=maxlen, 
                                                              ...
                                                              embed_dim=self.configuration[EMBEDDING_SIZE_KEY])
        self.att = layers.MultiHeadAttention(num_heads=num_heads, 
                                             key_dim=self.configuration[EMBEDDING_SIZE_KEY])
        self.ffn = tf.keras.Sequential([layers.Dense(num_heads*4, activation="relu"), 
                                        layers.Dense(self.configuration[EMBEDDING_SIZE_KEY])])
...
class train_distilbert_transformer(object):
    ...
    def train_model(self, train_dataset, val_datasets, test_datasets):
        ...
        model = distilbert_transformer(self.config,
                                     maxlen=self.maxlen,
