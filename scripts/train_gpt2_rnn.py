...
class gpt2_rnn(tf.keras.Model):
    ...
    def call(self, input_ids, **kwargs):
        ...
        #rnn
        if self.config["model_name"] == "gpt2_lstm":
            rnn_output = self.lstm(word_embeddings)
        elif self.config["model_name"] == "gpt2_bilstm":
            rnn_output = self.bilstm(word_embeddings)
        elif self.config["model_name"] == "gpt2_gru":
            rnn_output = self.gru(word_embeddings)
        elif self.config["model_name"] == "gpt2_bigru":
...
class train_gpt2_rnn(object):
    ...
    def train_model(self, train_dataset, val_datasets, test_datasets):
        ...
        model = gpt2_rnn(self.config).build_model(input_shape = train_dataset[0].shape[1])
        self.model = model
