...
class roberta_rnn(tf.keras.Model):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        self.configuration = AutoConfig.from_pretrained(MODEL).to_dict()
        self.encoder = TFRobertaModel.from_pretrained(MODEL)
    ...
    def call(self, input_ids, training, **kwargs):
        ...
        #rnn
        if self.config["model_name"] == "roberta_lstm":
            rnn_output = self.lstm(word_embeddings)
        elif self.config["model_name"] == "roberta_bilstm":
            rnn_output = self.bilstm(word_embeddings)
        elif self.config["model_name"] == "roberta_gru":
            rnn_output = self.gru(word_embeddings)
        elif self.config["model_name"] == "roberta_bigru":
...
class train_roberta_rnn(object):
    ...
    def train_model(self, train_dataset, val_datasets, test_datasets):
        ...
        model = roberta_rnn(self.config).build_model(input_shape = train_dataset[0].shape[1])
        self.model = model
