...
class roberta_cnn(tf.keras.Model):
    def __init__(self, config, **kwargs):
        ...
        self.conv_1 = layers.Conv2D(filters = self.config["hidden_units"], 
                                    kernel_size = (3, self.configuration["hidden_size"]),
                                    strides = 1,
                                    dilation_rate = 1,
                                    padding = "valid",
        ...
        self.conv_2 = layers.Conv2D(filters = self.config["hidden_units"], 
                                    kernel_size = (4, self.configuration["hidden_size"]),
                                    strides = 1,
                                    dilation_rate = 1,
                                    padding = "valid",
                                    data_format = "channels_first",
        ...
        self.conv_3 = layers.Conv2D(filters = self.config["hidden_units"], 
                                    kernel_size = (5, self.configuration["hidden_size"]),
                                    strides = 1,
                                    dilation_rate = 1,
                                    padding = "valid",
                                    data_format = "channels_first",
...
class train_roberta_cnn(object):
    ...
    def train_model(self, train_dataset, val_datasets, test_datasets):
        ...
        model = roberta_cnn(self.config).build_model(input_shape = train_dataset[0].shape[1])
        self.model = model
