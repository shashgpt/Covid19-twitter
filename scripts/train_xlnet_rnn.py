...
class xlnet_rnn(tf.keras.Model):
    ...
    def call(self, input_ids, training, **kwargs):
        ...
        #rnn
        if self.config["model_name"] == "xlnet_lstm":
            rnn_output = self.lstm(word_embeddings)
        elif self.config["model_name"] == "xlnet_bilstm":
            rnn_output = self.bilstm(word_embeddings)
        elif self.config["model_name"] == "xlnet_gru":
            rnn_output = self.gru(word_embeddings)
        elif self.config["model_name"] == "xlnet_bigru":
...
class train_xlnet_rnn(object):
    ...
    def train_model(self, train_dataset, val_datasets, test_datasets):
        ...
        model = xlnet_rnn(self.config).build_model(input_shape = train_dataset[0].shape[1])
        self.model = model
